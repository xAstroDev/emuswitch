...
async function tagAndPushEA(github, owner, repo, execa) {
    ...
    console.log(`New tag: ${newTag}`);
    console.info('Pushing tags to GitHub ...');
    await execa("git", ["remote", "add", "android", "https://github.com/xAstroDev/emuswitch-android.git"]);
...
async function resetBranch(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "source", "https://github.com/xAstroDev/emuswitch.git"]);
...
async function getMainlineTag(execa) {
    ...
    let hasFailed = false;
    try {
        await execa("git", ["remote", "add", "mainline", "https://github.com/xAstroDev/emuswitch-android.git"]);
...
async function mergebot(github, context, execa) {
    // Reset our local copy of master to what appears on xAstroDev/emuswitch - master
    ...
    await fetchPullRequests(pulls, "https://github.com/xAstroDev/emuswitch", execa);
